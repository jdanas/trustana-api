  ...
  const renderTreeNode = (node: CategoryTreeNode, depth = 0) => {
    ...
    return (
      <div key={node.id} className="select-none">
        ...
        >
          <div className="flex items-center flex-1" onClick={() => handleCategoryClick(node.id)}>
            {hasChildren ? (
              <button
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-red-500 text-center py-4">
            Error loading category tree: {error instanceof Error ? error.message : 'Unknown error'}
          </div>
        </CardContent>
      </Card>
    );
